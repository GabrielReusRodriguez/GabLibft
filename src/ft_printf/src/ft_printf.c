...
/*   Updated: 2024/03/12 23:55:53 by gabriel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_formated_data_2_string(va_list argp, char data_type, int fd)
{
	if (data_type == 'c')
...
static int	ft_print_special_char(va_list argp, int fd)
{
	return (ft_print_formated_data(argp, fd));
}
...
int	ft_printf_fd(int fd, char const *str, ...)
{
	...
	while (str[i] != '\0')
	{
		...
		{
			i++;
			printed_bytes = ft_print_special_char(argp, fd);
		}
		if (printed_bytes < 0)
...
int	ft_printf(char const *str, ...)
{
	...
	while (str[i] != '\0')
	{
		...
		{
			i++;
			printed_bytes = ft_print_special_char(argp, STDOUT_FILENO);
		}
		if (printed_bytes < 0)
			return (printed_bytes);
		num_bytes = num_bytes + printed_bytes;
